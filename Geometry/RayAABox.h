...
f_inline UVec4 RayIsParallel(const Vec3 &inDirection)
{
	// if (abs(inDirection) <= Epsilon) the ray is nearly parallel to the slab.
	return Vec3::sLessOrEqual(inDirection.Abs(), Vec3::sReplicate(1.0e-20f));
}
...
f_inline float RayAABox(const Vec3 &inOrigin, const Vec3 &inInvDirection, const UVec4 &inIsParallel, const Vec3 &inBoundsMin, const Vec3 &inBoundsMax)
{
	// Constants
	...
	Vec3 t1 = (inBoundsMin - inOrigin) * inInvDirection;
	Vec3 t2 = (inBoundsMax - inOrigin) * inInvDirection;

	// Compute the max of min(t1,t2) and the min of max(t1,t2) ensuring we don't
	// use the results from any directions parallel to the slab.
	Vec3 t_min = Vec3::sSelect(Vec3::sMin(t1, t2), flt_min, inIsParallel);
	Vec3 t_max = Vec3::sSelect(Vec3::sMax(t1, t2), flt_max, inIsParallel);

	// t_min.xyz = maximum(t_min.x, t_min.y, t_min.z);
	t_min = Vec3::sMax(t_min, t_min.Swizzle<SWIZZLE_Y, SWIZZLE_Z, SWIZZLE_X, SWIZZLE_W>());
	t_min = Vec3::sMax(t_min, t_min.Swizzle<SWIZZLE_Z, SWIZZLE_X, SWIZZLE_Y, SWIZZLE_W>());

	// t_max.xyz = minimum(t_max.x, t_max.y, t_max.z);
	t_max = Vec3::sMin(t_max, t_max.Swizzle<SWIZZLE_Y, SWIZZLE_Z, SWIZZLE_X, SWIZZLE_W>());
	t_max = Vec3::sMin(t_max, t_max.Swizzle<SWIZZLE_Z, SWIZZLE_X, SWIZZLE_Y, SWIZZLE_W>());
	...
	// if (inIsParallel && !(Min <= inOrigin && inOrigin <= Max)) return FLT_MAX; else return t_min;
	UVec4 no_parallel_overlap = UVec4::sOr(Vec3::sLess(inOrigin, inBoundsMin), Vec3::sGreater(inOrigin, inBoundsMax));
	no_intersection = UVec4::sOr(no_intersection, UVec4::sAnd(inIsParallel, no_parallel_overlap));
...
// Note: Can return negative value if ray starts in box
f_inline Vec4 RayAABox4(const Vec3 &inOrigin, const Vec3 &inInvDirection, const UVec4 &inIsParallel, const Vec4 &inBoundsMinX, const Vec4 &inBoundsMinY, const Vec4 &inBoundsMinZ, const Vec4 &inBoundsMaxX, const Vec4 &inBoundsMaxY, const Vec4 &inBoundsMaxZ)
{
	// Constants
	...
	// Parallel
	UVec4 parallelx = inIsParallel.SplatX();
	UVec4 parallely = inIsParallel.SplatY();
	UVec4 parallelz = inIsParallel.SplatZ();

	// Inverse direction
	Vec4 invdirx = inInvDirection.SplatX();
	Vec4 invdiry = inInvDirection.SplatY();
	Vec4 invdirz = inInvDirection.SplatZ();
...
f_inline void RayAABox(const Vec3 &inOrigin, const Vec3 &inInvDirection, const UVec4 &inIsParallel, const Vec3 &inBoundsMin, const Vec3 &inBoundsMax, float &outMin, float &outMax)
{
	// Constants
	Vec3 flt_min = Vec3::sReplicate(-FLT_MAX);
	Vec3 flt_max = Vec3::sReplicate(FLT_MAX);
...
f_inline bool RayAABoxHits(const Vec3 &inOrigin, const Vec3 &inInvDirection, const UVec4 &inIsParallel, const Vec3 &inBoundsMin, const Vec3 &inBoundsMax, float inClosest)
{
	// Constants
	Vec3 flt_min = Vec3::sReplicate(-FLT_MAX);
	Vec3 flt_max = Vec3::sReplicate(FLT_MAX);
	...
	// if (inIsParallel && !(Min <= inOrigin && inOrigin <= Max)) return false; else return true;
	UVec4 no_parallel_overlap = UVec4::sOr(Vec3::sLess(inOrigin, inBoundsMin), Vec3::sGreater(inOrigin, inBoundsMax));
	no_intersection = UVec4::sOr(no_intersection, UVec4::sAnd(inIsParallel, no_parallel_overlap));
...
f_inline bool RayAABoxHits(const Vec3 &inOrigin, const Vec3 &inDirection, const Vec3 &inBoundsMin, const Vec3 &inBoundsMax)
{
	...
	Vec3 abs_dir_yzz = abs_dir.Swizzle<SWIZZLE_Y, SWIZZLE_Z, SWIZZLE_Z, SWIZZLE_W>();
	Vec3 abs_dir_xyx = abs_dir.Swizzle<SWIZZLE_X, SWIZZLE_Y, SWIZZLE_X, SWIZZLE_W>();

	Vec3 extents_yzz = extents.Swizzle<SWIZZLE_Y, SWIZZLE_Z, SWIZZLE_Z, SWIZZLE_W>();
	Vec3 extents_xyx = extents.Swizzle<SWIZZLE_X, SWIZZLE_Y, SWIZZLE_X, SWIZZLE_W>();

	Vec3 diff_yzx = diff.Swizzle<SWIZZLE_Y, SWIZZLE_Z, SWIZZLE_X, SWIZZLE_W>();

	Vec3 dir_yzx = inDirection.Swizzle<SWIZZLE_Y, SWIZZLE_Z, SWIZZLE_X, SWIZZLE_W>();
