...
{
	assert(mCurrentPosition > 0); // First call GetFirstKey() to get the first key, then you can convert it to a VK code
...
{	
	...
	assert(mCurrentPosition > 0); // First call GetFirstKey() to get the first key, then you can convert it to an ASCII code
	int key = mDOD[mCurrentPosition - 1].dwOfs;
